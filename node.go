...
func (me *Node) Copy() *Node {
	...
	if me.Left != nil {
		newNode.Left = me.Left.Copy()
	}
	if me.Right != nil {
		newNode.Right = me.Right.Copy()
	}
	return newNode
}
...
func (me *Node) Next(key Comparable) *Node {
	...
	if !node.Key.EqualTo(key) {
		...
		for i := len(stack) - 1; i >= 0; i-- {
			if stack[i].Key.GreaterThan(key) {
				return stack[i]
			}
		}
...
func (me *Node) Balance() *Node {
	...
	if me.Left != nil {
		me.Left = me.Left.Balance()
	}
	if me.Right != nil {
		me.Right = me.Right.Balance()
	}
	return me
}
...
func (me *Node) WalkRangeForward(iterator func(me *Node), from Comparable, to Comparable) {
	if me.Left != nil {
		me.Left.WalkRangeForward(iterator, from, to)
	}
	if !me.Key.LessThan(from) && !me.Key.GreaterThan(to) {
		iterator(me)
	}
	if me.Right != nil {
		me.Right.WalkRangeForward(iterator, from, to)
	}
}
...
func (me *Node) WalkRangeBackward(iterator func(me *Node), from Comparable, to Comparable) {
	if me.Right != nil {
		me.Right.WalkRangeBackward(iterator, from, to)
	}
	if !me.Key.LessThan(from) && !me.Key.GreaterThan(to) {
		iterator(me)
	}
	if me.Left != nil {
		me.Left.WalkRangeBackward(iterator, from, to)
	}
}
